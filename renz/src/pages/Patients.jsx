...
export default function Patients() {
  ...
  const [editingPatient, setEditingPatient] = useState(null); // null = create mode
  ...
  return (
    <div className="patients-page animate-fade-in">
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        ...
        <div className="patients-toolbar">
          ...
          <div className="patients-toolbar-meta">
            Showing <strong>{showingStart === 0 ? 0 : `${showingStart}-${showingEnd}`}</strong> of <strong>{filteredPatients.length}</strong> patients
          </div>
        </div>
          ...
          <table>
            ...
            <tbody>
              {paginatedPatients.length === 0 ? (
                 <tr><td colSpan="8" style={{textAlign: 'center', padding: '2rem', color: '#6b7280'}}>No patients found.</td></tr>
                ...
                <tr key={p.id}>
                  ...
                  <td style={{ textAlign: 'right' }}>
                    <div className="action-group">
                      <button className="icon-btn view" onClick={() => setViewPatient(p)} title="View Details"><Eye size={18} /></button>
                      <button className="icon-btn edit" onClick={() => openEditForm(p)} title="Edit Patient"><Edit size={18} /></button>
                      <button className="icon-btn delete" onClick={() => setPatientToDelete(p)} title="Delete Patient"><Trash2 size={18} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {/* 1. Create / Edit Patient Modal */}
      {isFormOpen && (
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="modal-header">
              <h3 style={{ fontWeight: 'bold' }}>{editingPatient ? 'Edit Patient' : 'Add New Patient'}</h3>
              <button onClick={() => setIsFormOpen(false)} style={{ border: 'none', background: 'none', cursor: 'pointer' }}><X size={20} /></button>
            </div>
      ...
      {/* 2. View Patient Modal */}
      {viewPatient && (
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="modal-header">
              <h3 style={{ fontWeight: 'bold' }}>Patient Details</h3>
              <button onClick={() => setViewPatient(null)} style={{ border: 'none', background: 'none', cursor: 'pointer' }}><X size={20} /></button>
            </div>
            <div className="modal-body">
              <div style={{ display: 'grid', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #eee', paddingBottom: '0.5rem' }}>
                  <span style={{ color: 'var(--text-light)' }}>Last Visit</span>
                  <span>{viewPatient.lastVisit}</span>
                </div>
        ...
        <div className="modal-overlay">
          <div className="modal-content">
             <div className="modal-header">
              ...
              <button onClick={() => setPatientToDelete(null)} style={{ border: 'none', background: 'none', cursor: 'pointer' }}><X size={20} /></button>
            </div>
            <div className="modal-body">
              <p style={{ color: 'var(--text-dark)' }}>
                Are you sure you want to delete <strong>{patientToDelete.name}</strong>? This action cannot be undone and will remove all associated records.
              </p>
            </div>
            <div className="modal-footer">
